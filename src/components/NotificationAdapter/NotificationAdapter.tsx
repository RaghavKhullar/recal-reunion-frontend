...
const NotificationAdapter = ({ rem, toggleNotification }: { rem: Rem, toggleNotification: () => void }) => {
    ...
    const changeRemPrivacy = async () => {
        setIsFetching(true);
        const changeRemPrivacyDispatch = await dispatch(changePrivacy({ id: rem.id, privacy: rem.isPrivate }));
        if (changePrivacy.fulfilled.match(changeRemPrivacyDispatch)) {
            if (changeRemPrivacyDispatch.payload.status === 200) {
                ...
                showNotification("Error", changeRemPrivacyDispatch.payload.data.message, "error")
            } else {
