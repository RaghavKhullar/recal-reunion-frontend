    ...
    const getWrittenRemOfPair = async () => {
        ...
        if (getRemOfPair.fulfilled.match(getRemOfPairDispatch)) {
            if (getRemOfPairDispatch.payload.status === 200) {
                ...
                if (getRemOfPairDispatch.payload.data.data.image && getRemOfPairDispatch.payload.data.data.image.length > 0) {
                    try {
                        const response = await fetch(BACKEND_URL + '/images/memory/' + getRemOfPairDispatch.payload.data.data.image, { method: "get", mode: "cors" });
        ...
        <>
            ...
            <div className={styles.coverRem}>
                <div style={{ display: "flex", alignItems: "center" }}>
                    <img src={arrow} style={{ width: '3rem', paddingBottom: "0.3rem", cursor: "pointer" }} onClick={() => { navigate("/user/" + id) }} /> <h1 className={styles.remheading}>EDIT A REM ABOUT <span className={styles.red}>{name}</span></h1>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <div>
                        <div style={{ padding: "2rem", backgroundColor: "#A72343", marginTop: "2rem", boxShadow: "0px 4px 4px 0px rgba(0, 0, 0, 0.35)", width: "34rem", height: "34rem" }}>
                            ...
                            <img src={file ? URL.createObjectURL(file) : BACKEND_URL + '/images/profiles/' + (image && image.length > 0 ? image : "temp")} style={{ maxWidth: "30rem", border: "0.2rem solid white", maxHeight: "30rem" }} />
                        </div>
                    ...
                    <div style={{ textAlign: "center" }}>
                        ...
                        <Textarea onChange={(e) => setContent(e.target.value)} value={content} styles={{
                            input: { height: '32rem', width: "40rem", backgroundColor: "transparent", border: "3px solid #411D76", borderRadius: "10px", fontFamily: "'Fira Sans', sans-serif", fontSize: "1.5rem", overflowY: "auto" },
                        }} />
