...
const OtherUserProfile: React.FC = () => {
    ...
    const [otherUserDetails, setOtherUserDetails] = useState<GetUserDetailsResponse>();
    ...
    const fetchPublicRems = async (id: string) => {
        ...
        if (getPublicRemsOfUser.fulfilled.match(getPublicRemsOfUserDispatch)) {
            if (getPublicRemsOfUserDispatch.payload.status === 200) {
                if (Array.isArray(getPublicRemsOfUserDispatch.payload.data.writtenForUser)) {
                    setRemDetailsForUser(getPublicRemsOfUserDispatch.payload.data.writtenForUser);
                }
                if (Array.isArray(getPublicRemsOfUserDispatch.payload.data.writtenByUser)) {
                    setRemDetailsByUser(getPublicRemsOfUserDispatch.payload.data.writtenByUser);
                }
            } else {
    ...
    return (
        <>
            <div className={style.upper}>
                <div className={style.batch}>
                    ...
                    <div className={style.frame}>
                        {
                            otherUserDetails && otherUserDetails.oldRem && otherUserDetails.oldRem.image && otherUserDetails.oldRem.image.length !== 0 &&
                            <div className={style.before}>
                                <img className={style.anchorCircle} src={circle} />
                                <div className={style.internal}>
                                    <img src={`${BACKEND_URL}/images/profiles/${otherUserDetails.oldRem.image}`} />
                                </div>
                                <p className={style.textBefore}>Before</p>
                            </div>
                        }
                        {
                            otherUserDetails && otherUserDetails.user && otherUserDetails.user.image && otherUserDetails.user.image.length !== 0 &&
                            < div className={style.after}>
                                <img className={style.anchorCircle} src={circle} />
                                <div className={style.internal}>
                                    <img src={`${BACKEND_URL}/images/profiles/${otherUserDetails.user.image}`} />
                                </div>
            ...
            {(otherUserDetails !== undefined && remDetailsForUser.length !== 0 || remDetailsByUser.length !== 0) &&
                <div className={style.background}>
                    <div className={style.dome}>
                        <div className={style.domeShape}></div>
                    </div>
                    <div className={style.cardBackground}>
                        {remDetailsForUser.length !== 0 && <Card
                            remDetails={remDetailsForUser.slice(0, Math.min(remDetailsForUser.length, 6))}
                            head1={`Here’s what ${otherUserDetails?.user.name}'s friends think`}
                            head2={`Thoughts from ${otherUserDetails?.user.name}'s friends`}
                            writtenRems={false}
                        />}

                        {remDetailsByUser.length !== 0 && <Card
                            remDetails={remDetailsByUser.slice(0, Math.min(remDetailsByUser.length, 6))}
